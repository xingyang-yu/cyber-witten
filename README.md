...
- **Reranking is opt-in, and it sharpens ranking rather than recall.** `--rerank` (ask.py, run_eval.py, the local app) re-scores a 100-candidate cosine pool with BGE-reranker-v2-m3. Measured on the gold set: recall@8 is unchanged (0.95 → 0.93 — the bi-encoder already surfaces the right papers), but the expected paper's *rank* improves sharply (e.g. 5th→1st, 6th→1st on several questions), which matters for models that anchor on early passages. Costs a ~2.3GB one-time download and a few seconds per query, hence off by default.
