...
- **Reranking is opt-in, and it sharpens ranking rather than recall.** `--rerank` (ask.py, run_eval.py, the local app) re-scores a 100-candidate cosine pool with BGE-reranker-v2-m3. Measured on the gold set: recall@8 is a wash (0.94 for both — the bi-encoder already surfaces the right papers), but the expected paper's *rank* improves sharply (e.g. 5th→1st, 6th→1st on several questions), which matters for models that anchor on early passages. The measurement also caught a gold-set gap: the reranker promoted a genuinely on-topic paper (2303.02837) that the expected-citations list had missed — and that only the reranker surfaces in top-8. Costs a ~2.3GB one-time download and a few seconds per query, hence off by default.
...
- **The refusal gate detects topical coverage, not answer presence.** Calibrated on 26 out-of-corpus probes (23 gate-fair) vs 20 in-corpus questions (`evals/calibrate_refusal.py`, probe set in `evals/gold/probes.jsonl`): questions whose *topic* is absent (experimental measurements, biography, fabricated results) score below zero and are refused before generation. The default threshold 0.0 refuses 14 of 23 fair probes with zero false refusals and a 0.49 margin to the weakest in-corpus question; the aggressive zero-false setting (0.46) catches everything except three physicist-confirmed *near-miss* probes (other authors' constructions on topics Witten wrote about) — invisible to a topical gate by design, and handled downstream by the strict prompt, the citation guardrail, and human scoring. The suspect-flagging loop also earned its keep: of 7 flagged probes, physics review converted one into an in-corpus gold question outright (the corpus really covers Perelman's Ricci-flow program via 2404.19526 — the converted question scores 3.50, squarely in-corpus), marked three `gate_calibration: false` (mixed experimental questions, corpus-covered topics via Nekrasov-Witten 1002.0888, temporal/premise falseness), and confirmed three as valid near-misses. Probes can be wrong in both directions; the instrument audits its own benchmark.
