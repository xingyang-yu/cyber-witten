...
- **The refusal gate detects topical coverage, not answer presence.** Calibrated on 27 out-of-corpus probes vs 19 in-corpus questions (`evals/calibrate_refusal.py`, probe set in `evals/gold/probes.jsonl`): questions whose *topic* is absent (experimental measurements, biography, fabricated results) score below zero and are refused before generation; near-miss probes about adjacent content (other authors' theorems on topics Witten wrote about, false premises wrapped in real corpus vocabulary) score as topically relevant and pass the gate by design — those fall through to the strict prompt, the citation guardrail, and human scoring. On clean probes the distributions separate, but the margin to the weakest in-corpus question is thin (0.44 vs 0.49), so the default threshold stays a conservative 0.0 (catches ~70% of clean probes, never refuses an answerable question). A bonus: calibration flagged 7 suspect probes, and inspection showed two were *actually answerable* — the corpus contains papers the probe author didn't know (2404.19526 engages Perelman's Ricci-flow program; 1002.0888 is Nekrasov-Witten on the Omega deformation). The instrument audits its own benchmark.
